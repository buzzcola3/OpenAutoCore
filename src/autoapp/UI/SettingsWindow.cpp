...
namespace f1x::openauto::autoapp::ui {
  ...
  void SettingsWindow::show_tab4() {
      ...
      for (const auto &iface : interfaces) {
        ...
        for (const auto &entry : addressEntries) {
          if (entry.ip().protocol() == QAbstractSocket::IPv4Protocol) {
            entries << QStringLiteral("%1 %2").arg(iface.humanReadableName(), entry.ip().toString());
          }
        }
      }

      QFile infoFile(QStringLiteral("/tmp/openauto_settings_network"));
    ...
    void SettingsWindow::on_pushButtonNetwork0_clicked() {
      const int exitCode = QProcess::execute(QStringLiteral("/bin/sh"),
                                             {QStringLiteral("-c"), QStringLiteral("touch /tmp/openauto_network0")});
      Q_UNUSED(exitCode);
      updateNetworkInfo();
    }
    ...
    void SettingsWindow::on_pushButtonNetwork1_clicked() {
      const int exitCode = QProcess::execute(QStringLiteral("/bin/sh"),
                                             {QStringLiteral("-c"), QStringLiteral("touch /tmp/openauto_network1")});
      Q_UNUSED(exitCode);
      updateNetworkInfo();
    }
