    ...
    namespace autoapp {
      namespace service {
        ...
        void AndroidAutoEntity::onServiceDiscoveryRequest(
          ...
          const std::string cfgPath = "configuration/ServiceDiscoveryResponse.textproto";

          std::ifstream in(cfgPath);
          if (in) {
            std::ostringstream ss;
            ss << in.rdbuf();
            if (google::protobuf::TextFormat::ParseFromString(ss.str(), &serviceDiscoveryResponse)) {
          ...
          OPENAUTO_LOG(debug) << "[AndroidAutoEntity] " << serviceDiscoveryResponse.DebugString();
