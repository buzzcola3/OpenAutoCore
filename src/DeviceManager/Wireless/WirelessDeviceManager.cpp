...
struct DmDbusReadyWaiter { bool ready = false; };

void dmDbusReadyHandler(void* ud) {
    static_cast<DmDbusReadyWaiter*>(ud)->ready = true;
}
...
void WirelessDeviceManager::wakeEventFd() {
    uint64_t val = 1;
...
void WirelessDeviceManager::pollTCPListener() {
    ...
    DM_LOG(info) << "WirelessDeviceManager: WiFi client connected from " << addrStr;

    if (onWifiClientConnected) {
        auto connection = std::make_shared<TCPDeviceConnection>(clientFd);
        onWifiClientConnected(deviceId_, std::move(connection));
    } else {
        ::close(clientFd);
    }
}
