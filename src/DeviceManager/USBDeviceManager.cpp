...
void USBDeviceManager::setupHotplugWakeup() {
    ...
    l_io_set_read_handler(wakeupIo_, onHotplugWakeup, this, nullptr);
}

void USBDeviceManager::teardownHotplugWakeup() {
    if (wakeupIo_) {
...
bool USBDeviceManager::onHotplugWakeup(struct l_io*, void* userData) {
    auto* self = static_cast<USBDeviceManager*>(userData);
    uint64_t val;
...
void USBDeviceManager::wakeEllThread() {
    uint64_t val = 1;
...
void USBDeviceManager::advanceAoapSetup(AoapSetup* setup) {
    ...
    if (setup->state == 0) {
        ...
        size_t len = std::strlen(str.value) + 1;
...
void USBDeviceManager::registerLibusbFds() {
...
void USBDeviceManager::unregisterLibusbFds() {
    if (usbContext_) {
...
void USBDeviceManager::onLibusbFdAdded(int fd, short events, void* userData) {
    auto* self = static_cast<USBDeviceManager*>(userData);
...
void USBDeviceManager::onLibusbFdRemoved(int fd, void* userData) {
    auto* self = static_cast<USBDeviceManager*>(userData);
...
bool USBDeviceManager::onLibusbFdReady(struct l_io*, void* userData) {
    auto* self = static_cast<USBDeviceManager*>(userData);
    self->handleLibusbEvents();
    return true;
}

void USBDeviceManager::handleLibusbEvents() {
