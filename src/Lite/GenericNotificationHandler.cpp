...
constexpr const char* kTag = "[LiteGenericNotification]";
}

namespace aasdk::lite {

GenericNotificationHandler::GenericNotificationHandler(SendFn sender)
    : send_(std::move(sender)) {}

void GenericNotificationHandler::operator()(const InMessage& msg) {
    ++messageCount_;
    ...
    switch (msgId) {
        ...
        case GenericNotification::GENERIC_NOTIFICATION_SUBSCRIBE: {
            aap_protobuf::service::genericnotification::message::GenericNotificationSubscribe sub;
            if (sub.ParseFromArray(data, static_cast<int>(size)))
                AASDK_LOG(debug) << kTag << " Subscribe: " << sub.ShortDebugString();
            break;
        }
        case GenericNotification::GENERIC_NOTIFICATION_UNSUBSCRIBE: {
            aap_protobuf::service::genericnotification::message::GenericNotificationUnsubscribe unsub;
            if (unsub.ParseFromArray(data, static_cast<int>(size)))
                AASDK_LOG(debug) << kTag << " Unsubscribe: " << unsub.ShortDebugString();
            break;
        }
        case GenericNotification::GENERIC_NOTIFICATION_MESSAGE: {
            ...
            if (notification.ParseFromArray(data, static_cast<int>(size)))
                AASDK_LOG(debug) << kTag << " Message: " << notification.ShortDebugString();
            break;
        }
        case GenericNotification::GENERIC_NOTIFICATION_ACK: {
            ...
            if (ack.ParseFromArray(data, static_cast<int>(size)))
                AASDK_LOG(debug) << kTag << " Ack: " << ack.ShortDebugString();
            break;
        }
        default:
...
void GenericNotificationHandler::handleChannelOpenRequest(const InMessage& msg,
...
void GenericNotificationHandler::sendProto(messenger::ChannelId ch,
