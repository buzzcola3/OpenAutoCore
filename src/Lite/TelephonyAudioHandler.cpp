...
// TODO: add dedicated telephony msg type to transport wire protocol.
constexpr auto kTransportMsgType = buzz::wire::MsgType::SYSTEM_AUDIO;

} // namespace

namespace aasdk::lite {

TelephonyAudioHandler::TelephonyAudioHandler(
    SendFn sender,
    std::shared_ptr<buzz::autoapp::Transport::Transport> transport)
    : send_(std::move(sender)), transport_(std::move(transport)) {}

void TelephonyAudioHandler::operator()(const InMessage& msg) {
    ++messageCount_;
    const auto& payload = msg.payload;

    if (payload.size() < 2) {
        AASDK_LOG(error) << "[LiteTelephonyAudio] payload too small";
        return;
    }
    ...
    switch (msgId) {
        ...
        case Media::MEDIA_MESSAGE_START: {
            ...
            if (start.ParseFromArray(data, static_cast<int>(size))) {
                ...
                AASDK_LOG(debug) << "[LiteTelephonyAudio] MediaStart: session=" << sessionId_;
            }
            break;
        }
        case Media::MEDIA_MESSAGE_STOP: {
            ...
            if (stop.ParseFromArray(data, static_cast<int>(size))) {
                AASDK_LOG(debug) << "[LiteTelephonyAudio] MediaStop: " << stop.ShortDebugString();
            }
            break;
        }
        ...
        case Media::MEDIA_MESSAGE_AUDIO_UNDERFLOW_NOTIFICATION:
            AASDK_LOG(warning) << "[LiteTelephonyAudio] Audio underflow notification";
            break;
        default:
            AASDK_LOG(debug) << "[LiteTelephonyAudio] unhandled msgId=" << msgId;
            break;
    }
}

void TelephonyAudioHandler::handleChannelOpenRequest(const InMessage& msg,
                                                     const uint8_t* data, size_t size) {
    aap_protobuf::service::control::message::ChannelOpenRequest request;
    if (!request.ParseFromArray(data, static_cast<int>(size))) {
        AASDK_LOG(error) << "[LiteTelephonyAudio] Failed to parse ChannelOpenRequest";
        return;
    }
    AASDK_LOG(debug) << "[LiteTelephonyAudio] ChannelOpenRequest: " << request.ShortDebugString();
...
void TelephonyAudioHandler::handleMediaSetup(const InMessage& msg,
                                             const uint8_t* data, size_t size) {
    aap_protobuf::service::media::shared::message::Setup setup;
    if (!setup.ParseFromArray(data, static_cast<int>(size))) {
        AASDK_LOG(error) << "[LiteTelephonyAudio] Failed to parse MediaSetup";
        return;
    }
    AASDK_LOG(info) << "[LiteTelephonyAudio] MediaSetup: codec="
    ...
    AASDK_LOG(debug) << "[LiteTelephonyAudio] MediaSetup response: " << config.ShortDebugString();
}

void TelephonyAudioHandler::handleCodecConfig(const InMessage& msg,
                                              const uint8_t* data, size_t size) {
    AASDK_LOG(debug) << "[LiteTelephonyAudio] codec config blob size=" << size;

    if (sessionId_ < 0) {
        AASDK_LOG(error) << "[LiteTelephonyAudio] session not set, cannot ACK codec config";
        return;
    }

    if (ensureTransportStarted()) {
        transport_->send(kTransportMsgType, 0, data, size);
    }
...
void TelephonyAudioHandler::handleMediaData(const InMessage& msg,
                                            const uint8_t* data, size_t size) {
    if (sessionId_ < 0) {
        AASDK_LOG(error) << "[LiteTelephonyAudio] session not set, cannot ACK media data";
        return;
    }
    ...
    if (ensureTransportStarted()) {
        transport_->send(kTransportMsgType, timestamp, frameData, frameSize);
    }
...
void TelephonyAudioHandler::sendProto(messenger::ChannelId ch,
...
bool TelephonyAudioHandler::ensureTransportStarted() {
    ...
    if (!transport_->startAsA(std::chrono::microseconds{1000})) {
        AASDK_LOG(error) << "[LiteTelephonyAudio] Failed to start transport";
        return false;
    }
    return transport_->isRunning();
}

uint64_t TelephonyAudioHandler::resolveTimestamp(bool hasTimestamp, uint64_t parsedTs) const {
