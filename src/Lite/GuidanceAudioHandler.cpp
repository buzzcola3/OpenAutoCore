...
GuidanceAudioHandler::GuidanceAudioHandler(
    SendFn sender,
    std::shared_ptr<buzz::autoapp::Transport::Transport> transport)
    : send_(std::move(sender)), transport_(std::move(transport)) {}

void GuidanceAudioHandler::operator()(const InMessage& msg) {
    ++messageCount_;
    const auto& payload = msg.payload;

    if (payload.size() < 2) {
        AASDK_LOG(error) << "[LiteGuidanceAudio] payload too small";
        return;
    }
    ...
    switch (msgId) {
        ...
        case Media::MEDIA_MESSAGE_START: {
            ...
            if (start.ParseFromArray(data, static_cast<int>(size))) {
                ...
                AASDK_LOG(debug) << "[LiteGuidanceAudio] MediaStart: session=" << sessionId_;
            }
            break;
        }
        case Media::MEDIA_MESSAGE_STOP: {
            ...
            if (stop.ParseFromArray(data, static_cast<int>(size))) {
                AASDK_LOG(debug) << "[LiteGuidanceAudio] MediaStop: " << stop.ShortDebugString();
            }
            break;
        }
        ...
        case Media::MEDIA_MESSAGE_AUDIO_UNDERFLOW_NOTIFICATION:
            AASDK_LOG(warning) << "[LiteGuidanceAudio] Audio underflow notification";
            break;
        default:
            AASDK_LOG(debug) << "[LiteGuidanceAudio] unhandled msgId=" << msgId;
            break;
    }
}

void GuidanceAudioHandler::handleChannelOpenRequest(const InMessage& msg,
                                                    const uint8_t* data, size_t size) {
    aap_protobuf::service::control::message::ChannelOpenRequest request;
    if (!request.ParseFromArray(data, static_cast<int>(size))) {
        AASDK_LOG(error) << "[LiteGuidanceAudio] Failed to parse ChannelOpenRequest";
        return;
    }
    AASDK_LOG(debug) << "[LiteGuidanceAudio] ChannelOpenRequest: " << request.ShortDebugString();
...
void GuidanceAudioHandler::handleMediaSetup(const InMessage& msg,
                                            const uint8_t* data, size_t size) {
    aap_protobuf::service::media::shared::message::Setup setup;
    if (!setup.ParseFromArray(data, static_cast<int>(size))) {
        AASDK_LOG(error) << "[LiteGuidanceAudio] Failed to parse MediaSetup";
        return;
    }
    AASDK_LOG(info) << "[LiteGuidanceAudio] MediaSetup: codec="
    ...
    AASDK_LOG(debug) << "[LiteGuidanceAudio] MediaSetup response: " << config.ShortDebugString();
}

void GuidanceAudioHandler::handleCodecConfig(const InMessage& msg,
                                             const uint8_t* data, size_t size) {
    AASDK_LOG(debug) << "[LiteGuidanceAudio] codec config blob size=" << size;

    if (sessionId_ < 0) {
        AASDK_LOG(error) << "[LiteGuidanceAudio] session not set, cannot ACK codec config";
        return;
    }
...
void GuidanceAudioHandler::handleMediaData(const InMessage& msg,
                                           const uint8_t* data, size_t size) {
    if (sessionId_ < 0) {
        AASDK_LOG(error) << "[LiteGuidanceAudio] session not set, cannot ACK media data";
        return;
    }
...
void GuidanceAudioHandler::sendProto(messenger::ChannelId ch,
...
bool GuidanceAudioHandler::ensureTransportStarted() {
    ...
    if (!transport_->startAsA(std::chrono::microseconds{1000})) {
        AASDK_LOG(error) << "[LiteGuidanceAudio] Failed to start transport";
        return false;
    }
    return transport_->isRunning();
}

uint64_t GuidanceAudioHandler::resolveTimestamp(bool hasTimestamp, uint64_t parsedTs) const {
