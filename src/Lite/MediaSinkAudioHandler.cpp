...
MediaSinkAudioHandler::MediaSinkAudioHandler(
    SendFn sender,
    std::shared_ptr<buzz::autoapp::Transport::Transport> transport)
    : send_(std::move(sender)), transport_(std::move(transport)) {}

void MediaSinkAudioHandler::operator()(const InMessage& msg) {
    ++messageCount_;
    const auto& payload = msg.payload;

    if (payload.size() < 2) {
        AASDK_LOG(error) << "[LiteMediaAudio] payload too small";
        return;
    }
    ...
    switch (msgId) {
        ...
        case Media::MEDIA_MESSAGE_START: {
            ...
            if (start.ParseFromArray(data, static_cast<int>(size))) {
                ...
                AASDK_LOG(debug) << "[LiteMediaAudio] MediaStart: session=" << sessionId_;
            }
            break;
        }
        case Media::MEDIA_MESSAGE_STOP: {
            ...
            if (stop.ParseFromArray(data, static_cast<int>(size))) {
                AASDK_LOG(debug) << "[LiteMediaAudio] MediaStop: " << stop.ShortDebugString();
            }
            break;
        }
        ...
        case Media::MEDIA_MESSAGE_AUDIO_UNDERFLOW_NOTIFICATION:
            AASDK_LOG(warning) << "[LiteMediaAudio] Audio underflow notification";
            break;
        default:
            AASDK_LOG(debug) << "[LiteMediaAudio] unhandled msgId=" << msgId;
            break;
    }
}

void MediaSinkAudioHandler::handleChannelOpenRequest(const InMessage& msg,
                                                     const uint8_t* data, size_t size) {
    aap_protobuf::service::control::message::ChannelOpenRequest request;
    if (!request.ParseFromArray(data, static_cast<int>(size))) {
        AASDK_LOG(error) << "[LiteMediaAudio] Failed to parse ChannelOpenRequest";
        return;
    }
    AASDK_LOG(debug) << "[LiteMediaAudio] ChannelOpenRequest: " << request.ShortDebugString();
...
void MediaSinkAudioHandler::handleMediaSetup(const InMessage& msg,
                                             const uint8_t* data, size_t size) {
    aap_protobuf::service::media::shared::message::Setup setup;
    if (!setup.ParseFromArray(data, static_cast<int>(size))) {
        AASDK_LOG(error) << "[LiteMediaAudio] Failed to parse MediaSetup";
        return;
    }
    AASDK_LOG(info) << "[LiteMediaAudio] MediaSetup: codec="
    ...
    AASDK_LOG(debug) << "[LiteMediaAudio] MediaSetup response: " << config.ShortDebugString();
}

void MediaSinkAudioHandler::handleCodecConfig(const InMessage& msg,
                                              const uint8_t* data, size_t size) {
    AASDK_LOG(debug) << "[LiteMediaAudio] codec config blob size=" << size;

    if (sessionId_ < 0) {
        AASDK_LOG(error) << "[LiteMediaAudio] session not set, cannot ACK codec config";
        return;
    }
...
void MediaSinkAudioHandler::handleMediaData(const InMessage& msg,
                                            const uint8_t* data, size_t size) {
    if (sessionId_ < 0) {
        AASDK_LOG(error) << "[LiteMediaAudio] session not set, cannot ACK media data";
        return;
    }
...
void MediaSinkAudioHandler::sendProto(messenger::ChannelId ch,
...
bool MediaSinkAudioHandler::ensureTransportStarted() {
    ...
    if (!transport_->startAsA(std::chrono::microseconds{1000})) {
        AASDK_LOG(error) << "[LiteMediaAudio] Failed to start transport";
        return false;
    }
    return transport_->isRunning();
}

uint64_t MediaSinkAudioHandler::resolveTimestamp(bool hasTimestamp, uint64_t parsedTs) const {
