...
constexpr const char* kTag = "[LiteVendorExtension]";
}

namespace aasdk::lite {

VendorExtensionHandler::VendorExtensionHandler(SendFn sender)
    : send_(std::move(sender)) {}

void VendorExtensionHandler::operator()(const InMessage& msg) {
    ++messageCount_;
    ...
    switch (msgId) {
        case Control::MESSAGE_CHANNEL_OPEN_REQUEST:
            ...
            break;
        default:
            AASDK_LOG(debug) << kTag << " msgId=" << msgId << " bytes=" << size;
            break;
    }
}

void VendorExtensionHandler::handleChannelOpenRequest(const InMessage& msg,
...
void VendorExtensionHandler::sendProto(messenger::ChannelId ch,
