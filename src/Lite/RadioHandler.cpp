...
constexpr const char* kTag = "[LiteRadio]";
}

namespace aasdk::lite {

RadioHandler::RadioHandler(SendFn sender)
    : send_(std::move(sender)) {}

void RadioHandler::operator()(const InMessage& msg) {
    ++messageCount_;
    ...
    switch (msgId) {
        case Control::MESSAGE_CHANNEL_OPEN_REQUEST:
            ...
            break;
        default:
            // Radio has many message types; log the ID and payload size
            AASDK_LOG(debug) << kTag << " msgId=" << msgId << " bytes=" << size;
            break;
    }
}

void RadioHandler::handleChannelOpenRequest(const InMessage& msg,
...
void RadioHandler::sendProto(messenger::ChannelId ch,
