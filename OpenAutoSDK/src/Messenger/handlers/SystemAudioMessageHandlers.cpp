...
constexpr const char* kLogPrefix = "[SystemAudioMessageHandlers]";

} // namespace

namespace aasdk::messenger::interceptor {

bool SystemAudioMessageHandlers::handle(const ::aasdk::messenger::Message& message) const {
  ++messageCount_;
  const auto& rawPayload = message.getPayload();

  if (rawPayload.size() <= ::aasdk::messenger::MessageId::getSizeOf()) {
    AASDK_LOG(error) << kLogPrefix << " system audio payload too small";
    return false;
  }
  ...
  switch (messageId.getId()) {
    ...
    case Media::MEDIA_MESSAGE_AUDIO_UNDERFLOW_NOTIFICATION:
      ...
      break;
    default:
      AASDK_LOG(debug) << kLogPrefix << " system audio message id=" << messageId.getId()
...
bool SystemAudioMessageHandlers::handleChannelOpenRequest(const ::aasdk::messenger::Message& message,
...
bool SystemAudioMessageHandlers::handleMediaData(const ::aasdk::messenger::Message& message,
                                                  const std::uint8_t* data,
                                                  std::size_t size) const {
  if (sender_ == nullptr) {
...
bool SystemAudioMessageHandlers::handleChannelSetupRequest(const ::aasdk::messenger::Message& message,
                                                            const std::uint8_t* data,
                                                            std::size_t size) const {
...
bool SystemAudioMessageHandlers::handleCodecConfig(const ::aasdk::messenger::Message& message,
                                                    const std::uint8_t* data,
                                                    std::size_t size) const {
...
void SystemAudioMessageHandlers::setMessageSender(
    std::shared_ptr<::aasdk::messenger::MessageSender> sender) {
  sender_ = std::move(sender);
}

void SystemAudioMessageHandlers::setTransport(
    std::shared_ptr<buzz::autoapp::Transport::Transport> transport) {
  transport_ = std::move(transport);
}

bool SystemAudioMessageHandlers::ensureTransportStarted() const {
...
uint64_t SystemAudioMessageHandlers::resolveTimestamp(bool hasTimestamp, uint64_t parsedTs) const {
  if (hasTimestamp) {
    return parsedTs;
  }
